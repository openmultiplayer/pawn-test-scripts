...
TEST__ M_01_CreateMenu()
{
    ...
    ASSERT_EQ(CreateMenu("This should fail", 2, 200.0, 100.0, 150.0, 150.0), -1);
...
{
    new Menu:menu;
    ASSERT_EQ(IsValidMenu(menu), 0);
    menu = CreateMenu("Menu", 0, 200.0, 100.0, 150.0, 150.0);
    ASSERT_EQ(IsValidMenu(menu), 1);
    DestroyMenu(menu);
    ASSERT_EQ(IsValidMenu(menu), 0);
    ASSERT_EQ(IsValidMenu(Menu:0), 0);
